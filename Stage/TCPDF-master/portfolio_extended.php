...
if ($extended == 0) {
    $sth = $pdo->prepare("SELECT $initials, $prefix, $lastName FROM person_cv WHERE id = $id");
    ...
    $sth = $pdo->prepare("SELECT first_name, prefix, last_name FROM person_cv WHERE id = $id");
    $sth->execute(array($id));
    $result = $sth->fetchAll();
    $j = 0;
    foreach ($result as $row) {
...
$sth = $pdo->prepare("SELECT name FROM role_profile INNER JOIN period ON role_profile.role_id = period.role_id WHERE id = $id ORDER BY period_date_till LIMIT 1;");
$sth->execute(array($id));
...
// Ambition blue side
$sth = $pdo->prepare("SELECT ambition FROM development_plan_ambition WHERE id = $id AND period_id = $id LIMIT 1");
...
$sth = $pdo->prepare("SELECT $name, $sector, $responsibleFor, $function, $locationWork, $reference, $referenceNumber, $dateFrom, $dateTill, $telephone FROM person_work INNER JOIN person_cv ON person_work.id = person_cv.id WHERE person_work.id='$id' ORDER BY date_till IS NULL DESC, date_from DESC LIMIT 1");
$sth->execute(array($id));
...
//    $pdf->Line($xgegevens, $yLine, 200, $yLine, $style);
    ...
    $pdf->SetTextColor(0, 0, 0);
    $pdf->MultiCell(33, 0, 'Plaats' . "\n" . 'Organisatie-type' . "\n" . "Verantwoordelijkheden" . "\n" . "Functie" . "\n" . "Referentie" . "\n", 0, 'R', 0, 0, 80, $y, true, 0, false, true, 20, 'M', true);
    //    $pdf->SetTextColor(0, 147, 214); Lichte kleur
    if ($color == $blauw) {
        $pdf->SetTextColor(0, 135, 203);
    }
    ...
    $pdf->MultiCell(33, 0, 'Datum' . "\n \n" . 'Rolprofiel' . "\n \n", 0, 'R', 0, 0, 80, $y, true, 0, false, true, 25, '', true);
    $pdf->SetFont('helvetica', '', 10);

    $pdf->MultiCell(70, 3, '' . ucfirst('Hier komt de datum') . "\n", 0, 'L', 0, 0, 125, $y, true, 0, false, false, 4, 'M', true);
    $y += 9;
    $pdf->MultiCell(70, 3, '' . ucfirst('Hier komt de rolprofiel') . "\n", 0, 'L', 0, 0, 125, $y, true, 0, false, false, 4, 'M', true);
    $y += 18;
    $pdf->SetFont('helvetica', 'B', 10);

    $pdf->MultiCell(33, 0, 'Context' . "\n", 0, 'R', 0, 0, 80, $y, true, 0, false, true, 75, '', true);

    $pdf->SetFont('helvetica', '', 10);

    $sth = $pdo->prepare("SELECT target FROM development_plan_target WHERE id ='2' AND period_id = $period_id limit 2");
    $sth->execute();
    $txtAmbitie = $sth->fetchAll();
    $y += 17;
    ...
    $pdf->MultiCell(33, 0, 'Doelstelling' . "\n", 0, 'R', 0, 0, 80, $y, true, 0, false, true, 75, '', true);
    foreach ($txtAmbitie as $row) {

        $pdf->SetFont('helvetica', '', 10);

        $pdf->MultiCell(80, 0, $row['target'] . "\n", 0, 'L', 0, 0, 125, $y, true, 0, false, false, 75, '', true);
        $y += 18;
    }

    $sth = $pdo->prepare("SELECT plan_of_action FROM software_talentpass.development_plan_plan_of_action WHERE id=2 ORDER BY period_id desc limit 2");
    $sth->execute();
    $txtAmbitie = $sth->fetchAll();
    foreach ($txtAmbitie as $row) {
        $pdf->SetFont('helvetica', 'B', 10);

        $pdf->MultiCell(33, 0, 'Actieplan' . "\n", 0, 'R', 0, 0, 80, $y, true, 0, false, true, 75, '', true);
        $pdf->SetFont('helvetica', '', 10);
    ...
    $sth = $pdo->prepare("SELECT result FROM software_talentpass.development_plan_result WHERE id=2 ORDER BY period_id DESC limit 2");
    $sth->execute();
    $txtAmbitie = $sth->fetchAll();
    foreach ($txtAmbitie as $row) {
        $pdf->SetFont('helvetica', 'B', 10);

        $pdf->MultiCell(33, 0, 'Resultaten' . "\n", 0, 'R', 0, 0, 80, $y, true, 0, false, true, 75, '', true);
        $pdf->SetFont('helvetica', '', 10);
    ...
    $sth = $pdo->prepare("SELECT learning_moment FROM software_talentpass.development_plan_learning_moment WHERE id=2 ORDER BY period_id LIMIT 2");
    $sth->execute();
    $txtAmbitie = $sth->fetchAll();
    foreach ($txtAmbitie as $row) {
        $pdf->SetFont('helvetica', 'B', 10);

        $pdf->MultiCell(33, 0, 'Leermomenten' . "\n", 0, 'R', 0, 0, 80, $y, true, 0, false, true, 75, '', true);
        $pdf->SetFont('helvetica', '', 10);
...
//    }
    ...
    // $style = array('width' => 0.2, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(0, 147, 214)); oude kleur
    if ($color == $blauw) {
