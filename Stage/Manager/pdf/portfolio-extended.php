...
//    $pdf->Line($xgegevens, $yLine, 200, $yLine, $style);
    ...
    $y += $yChange;
    $pdf->SetTextColor(0, 0, 0);
    $pdf->MultiCell($boxWidth, 0, '' . ucfirst($row[$sector]) . "\n", 0, 'L', 0, 0, 125, $y, true, 0, false, false, $boxHeight, '', true);
    $y += $yChange;
    if (strlen($row[$responsibleFor]) < 10) {
        $pdf->MultiCell($boxWidth, 0, '' . ucfirst($row[$responsibleFor]) . "\n", 0, 'L', 0, 0, 125, $y, true, 0, false, false, $boxHeight, '', true);
        $y += $yChange;
...
//        $pdf->SetFont('helvetica', '', 10); als ik het dikgedrukt wil.
        foreach ($txtAmbitie as $row) {
...
$pdf->Output('Portfolio extended.pdf', 'I');
